<template>
  <section class="aui-scrollView">
		...
		<div class="aui-account-input">
			<div class="b-line">
				<input type="text" placeholder="请输入手机号码" id="login_phone"  v-model="username">
				<!-- <label class="err err-top40" id="login_phone_text" style="display: block;">手机号不能为空</label> -->
			</div>
			<div class="">
				<input type="password" placeholder="请输入密码 " id="login_password" v-model="password">
				<!-- <label class="err err-top40" id="login_password_text" style="display: block;">密码不能为空</label> -->
			</div>
		</div>
		<div class="aui-account-button">
			<button id="login_submit" @click="reg">注册</button>
            {{mess}}
		</div>
  ...
  methods:{
    reg(){
      ...
      axios({
        ...
        res => {
            console.log(res)
            res.data.err===0 ? this.$router.push('/login') : this.mess = res.data.msg}
      )
...
.aui-account-input div {
    position: relative;
}
.aui-account-input input {
    height: 2.8rem;
    line-height: 2.8rem;
    padding: 0.8rem 1rem;
    font-size: 0.85rem;
